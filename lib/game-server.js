...
function attachGameServer(io) {
  ...
  class Game {
    constructor(id, mode = 'domination') {
      ...
      this.endReason = null;   // 'time' | 'target'
    ...
    /** False during the pre-match countdown */
    isLive() {
    ...
    tickProjectiles() {
      ...
      for (const [projectileId, projectile] of this.projectiles) {
        ...
        const damage = owner && !owner.isBot ? GAME_CONFIG.HUMAN_DAMAGE : GAME_CONFIG.DEFAULT_DAMAGE;
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('hitClaim', (data) => {
      try {
        ...
        const killed = game.handlePlayerHit(socket.id, data.targetId, GAME_CONFIG.HUMAN_DAMAGE);
        io.to(game.id).emit('playerHit', {
          attackerId: socket.id,
          targetId: data.targetId,
          damage: GAME_CONFIG.HUMAN_DAMAGE,
          killed,
