...
const GAME_CONFIG = {
  ...
  TEAM_SIZE: 3,
  RESPAWN_DELAY: 3000,
  MATCH_DURATION: 180000, // one round: 3 minutes
  ROUNDS_TO_WIN: 2,       // best of three
  MAX_ROUNDS: 3,
  INTERMISSION: 15000,    // ms between rounds
  LOW_PASS_ALTITUDE: 24,  // capture twice as fast below this (units)
  STEALTH_ALTITUDE: 18,   // below the treeline you vanish from radar beyond STEALTH_REVEAL
  STEALTH_REVEAL: 130,
  ...
  KILL_SCORE: 5,            // team points per shoot-down in Domination (windmills give 1 per tick)
  COUNTDOWN: 5000,          // ms of pre-match countdown once the first human joins
  TDM_TARGET: 30,           // team kills that win Team Deathmatch
  STREAK_RADAR: 3,          // killstreak rewards
  STREAK_WINGMAN: 5,
  STREAK_AIRSTRIKE: 7,
  WINGMAN_DURATION: 45000,  // ms an escort bot stays with its owner
  AIRSTRIKE_RADIUS: 160,
  AIRSTRIKE_DAMAGE: 60,
  ...
  HIT_RADIUS_HUMAN: 11,           // human shots get a more forgiving hit box
  HUMAN_DAMAGE: 25,               // human shots: four hits down a plane (bots do DEFAULT_DAMAGE)
  CLAIM_TOLERANCE: 45,            // max distance between a client-claimed impact and the server's view
...
// Plane classes: humans pick one; stats are enforced here
const PLANES = {
  interceptor: { maxHealth: 90,  damage: 20, captureWeight: 1.0, speed: 1.15, turn: 1.15, vision: 1.0 },
  bomber:      { maxHealth: 150, damage: 30, captureWeight: 1.6, speed: 0.9,  turn: 0.75, vision: 1.0 },
  scout:       { maxHealth: 100, damage: 22, captureWeight: 1.0, speed: 1.05, turn: 1.0,  vision: 1.6 },
};
const DEFAULT_PLANE = 'scout';
const PAINTS = ['classic', 'delft', 'tulip', 'midnight', 'gold'];
const TRAILS = ['white', 'tulip', 'orange', 'gold', 'delft'];
function cleanCosmetics(c) {
  return {
    paint: c && PAINTS.includes(c.paint) ? c.paint : 'classic',
    trail: c && TRAILS.includes(c.trail) ? c.trail : 'white',
    stork: !!(c && c.stork),
  };
}
function planeOf(p) { return PLANES[p && p.plane] || PLANES[DEFAULT_PLANE]; }
function planeDamage(p) { return planeOf(p).damage; }
...
function attachGameServer(io) {
   ...
   */
  class Game {
    constructor(id, mode = 'domination') {
      this.id = id;
      ...
      this.mode = mode;        // 'domination' | 'tdm'
      this.endReason = null;   // 'time' | 'target' (round) | 'rounds' (match)
      this.round = 1;
      this.roundWins = { red: 0, blue: 0 };
      this.phase = 'countdown'; // countdown | live | intermission | ended
    ...
    /** Fresh scores and windmills, everyone respawned, countdown, go */
    startRound(n) {
      this.round = n;
      this.suddenDeath = n >= GAME_CONFIG.MAX_ROUNDS;
      this.phase = 'countdown';
      this.startTime = Date.now() + GAME_CONFIG.COUNTDOWN;
      ...
      for (const [id, p] of this.players) {
        p.health = p.maxHealth || GAME_CONFIG.MAX_HEALTH;
        p.energy = GAME_CONFIG.MAX_ENERGY;
        p.streak = 0;
        p.position = this.getRandomSpawnPosition(p.team);
        p.lastPosition = null;
        io.to(this.id).emit('playerRespawn', { playerId: id, position: p.position, health: p.health, energy: p.energy });
      }
      io.to(this.id).emit('roundStart', {
        round: n, maxRounds: GAME_CONFIG.MAX_ROUNDS, roundWins: this.roundWins,
        countdown: GAME_CONFIG.COUNTDOWN, suddenDeath: this.suddenDeath,
      });
    }

    /** A round is over on time, or when a TDM team reaches the target */
    isRoundOver() {
      if (Date.now() - this.startTime >= GAME_CONFIG.MATCH_DURATION) { this.endReason = 'time'; return true; }
      if (this.mode === 'tdm' && Math.max(this.scores.red, this.scores.blue) >= GAME_CONFIG.TDM_TARGET) {
    ...
    endRound() {
      const winner = this.scores.red > this.scores.blue ? 'red' : this.scores.blue > this.scores.red ? 'blue' : null;
      if (winner) this.roundWins[winner]++;
      this.lastRound = { round: this.round, winner, scores: { ...this.scores }, reason: this.endReason };
      const decided = this.roundWins.red >= GAME_CONFIG.ROUNDS_TO_WIN || this.roundWins.blue >= GAME_CONFIG.ROUNDS_TO_WIN ||
        this.round >= GAME_CONFIG.MAX_ROUNDS;
      this.phase = 'intermission';
      this.intermissionUntil = Date.now() + GAME_CONFIG.INTERMISSION;
      io.to(this.id).emit('roundEnd', {
        round: this.round, winner, scores: this.scores, roundWins: this.roundWins, reason: this.endReason,
        mvp: this.mvp(), intermission: decided ? 0 : GAME_CONFIG.INTERMISSION, final: decided,
        nextSuddenDeath: !decided && this.round + 1 >= GAME_CONFIG.MAX_ROUNDS,
      });
      console.log(`Game ${this.id} round ${this.round}: ${winner || 'draw'} (${this.scores.red}-${this.scores.blue})`);
      if (decided) this.endMatch();
    }
    ...
    endMatch() {
      ...
      console.log(`Game ${this.id} ended — rounds Red ${this.roundWins.red} : Blue ${this.roundWins.blue}`);
    }

    /** False during the countdown, the intermission and after the match */
    isLive() {
      if (this.status === 'ended' || this.phase === 'intermission') return false;
      if (Date.now() >= this.startTime) { this.phase = 'live'; return true; }
      return false;
    }

    /** Player with the most kills (fewest deaths breaks ties) */
    mvp() {
      let best = null;
      for (const [, p] of this.players) {
        if (!best || (p.kills || 0) > (best.kills || 0) ||
            ((p.kills || 0) === (best.kills || 0) && (p.deaths || 0) < (best.deaths || 0))) best = p;
      }
      return best ? { id: best.id, username: best.username, kills: best.kills || 0, deaths: best.deaths || 0, isBot: !!best.isBot, team: best.team } : null;
    }

    /** Killstreak rewards, announced to the room */
    checkStreak(attacker) {
      const s = attacker.streak;
      let reward = null;
      if (s === GAME_CONFIG.STREAK_RADAR) reward = 'radar';
      else if (s === GAME_CONFIG.STREAK_WINGMAN) { reward = 'wingman'; this.spawnWingman(attacker); }
      else if (s === GAME_CONFIG.STREAK_AIRSTRIKE) { reward = 'airstrike'; this.airstrike(attacker); }
      if (s >= GAME_CONFIG.STREAK_RADAR) {
        io.to(this.id).emit('streak', { playerId: attacker.id, username: attacker.username, streak: s, reward });
      }
    }

    /** A temporary escort bot on the owner's team */
    spawnWingman(owner) {
      const id = `bot_${this.id}_w${this.botCounter++}`;
      this.addPlayer({
        id, username: `${owner.username} Wingman`,
        position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 },
      }, owner.team);
      const bot = this.players.get(id);
      bot.isBot = true;
      bot.escortOf = owner.id;
      bot.escortUntil = Date.now() + GAME_CONFIG.WINGMAN_DURATION;
      bot.position = { x: owner.position.x + 15, y: owner.position.y || 30, z: owner.position.z + 15 };
      bot.heading = 0;
      bot.waypoint = null;
      bot.waypointUntil = 0;
      bot.lastFire = 0;
    ...
    /** Heavy damage to every enemy near the attacker */
    airstrike(attacker) {
      for (const [id, target] of this.players) {
        if (id === attacker.id || target.team === attacker.team || target.health <= 0 || !target.position) continue;
        const dx = target.position.x - attacker.position.x;
        const dz = target.position.z - attacker.position.z;
        if (dx * dx + dz * dz > GAME_CONFIG.AIRSTRIKE_RADIUS * GAME_CONFIG.AIRSTRIKE_RADIUS) continue;
        const killed = this.handlePlayerHit(attacker.id, id, GAME_CONFIG.AIRSTRIKE_DAMAGE);
        io.to(this.id).emit('playerHit', {
          attackerId: attacker.id, targetId: id, damage: GAME_CONFIG.AIRSTRIKE_DAMAGE, killed,
          projectileId: null, airstrike: true,
          targetHealth: target.health, attackerKills: attacker.kills, targetDeaths: target.deaths,
          gameState: { scores: this.scores, timeRemaining: GAME_CONFIG.MATCH_DURATION - (Date.now() - this.startTime) },
        });
      }
    }
    ...
    fillBots() {
      while (this.players.size < GAME_CONFIG.PLAYERS_PER_GAME) {
        ...
        const bot = this.players.get(id);
        bot.isBot = true;
        bot.plane = ['interceptor', 'bomber', 'scout'][this.botCounter % 3];
     ...
     */
    tickBots() {
      if (this.status === 'ended' || !this.isLive()) return;
      const now = Date.now();
      ...
      for (const [id, bot] of this.players) {
        ...
        // Escort bots (wingman streak reward) expire, and leave with their owner
        if (bot.escortOf) {
          const owner = this.players.get(bot.escortOf);
          if (!owner || now > bot.escortUntil) {
            this.removePlayer(id);
            io.to(this.id).emit('playerLeft', id);
            continue;
          }
        ...
        for (const [oid, other] of this.players) {
          ...
          // Below the treeline you're off the bots' radar unless they're close
          if ((other.position.y || 30) < GAME_CONFIG.STEALTH_ALTITUDE) {
            const sdx = other.position.x - bot.position.x, sdz = other.position.z - bot.position.z;
            if (sdx * sdx + sdz * sdz > GAME_CONFIG.STEALTH_REVEAL * GAME_CONFIG.STEALTH_REVEAL) continue;
          }
        ...
        } else if (bot.escortOf) {
          // Wingman: hold formation off the owner's wing
          const owner = this.players.get(bot.escortOf);
          tx = owner.position.x + 18;
          tz = owner.position.z + 18;
          ty = Math.max(12, Math.min(GAME_CONFIG.BOT_MAX_ALTITUDE, (owner.position.y || 30) + 4));
        } else {
     ...
     */
    addPlayer(player, forcedTeam) {
      // Assign team based on current team sizes unless one is requested
      const team = forcedTeam || (this.teams.red.size <= this.teams.blue.size ? 'red' : 'blue');
      this.teams[team].add(player.id);
      this.players.set(player.id, {
        ...player,
        team,
        plane: PLANES[player.plane] ? player.plane : DEFAULT_PLANE,
        maxHealth: planeOf(player).maxHealth,
        health: planeOf(player).maxHealth,
    ...
    getRandomSpawnPosition(team) {
      // Hold a windmill and you respawn beside your longest-held one
      let anchor = null;
      for (const mill of this.windmills) {
        if (mill.team === team && (!anchor || mill.capturedAt < anchor.capturedAt)) anchor = mill;
      }
      const basePosition = anchor
        ? { x: anchor.x + (team === 'red' ? -70 : 70), y: 0, z: anchor.z + 40 }
        : team === 'red' ? { x: -50, y: 0, z: 0 } : { x: 50, y: 0, z: 0 };
     ...
     */
    tickProjectiles() {
      if (this.status === 'ended' || this.projectiles.size === 0 || !this.isLive()) return;
      ...
      for (const [projectileId, projectile] of this.projectiles) {
        ...
        const damage = owner && !owner.isBot ? planeDamage(owner) : GAME_CONFIG.DEFAULT_DAMAGE;

        // Human shots register on the shooter's screen (see 'hitClaim'), so
        // the server only moves them and expires them here
        ...
        for (const [targetId, target] of this.players) {
          ...
          if (dist <= hitRadius) {
            ...
            const killed = this.handlePlayerHit(projectile.ownerId, targetId, damage);
    ...
    handlePlayerHit(attackerId, targetId, damage) {
      ...
      const validDamage = Math.min(Math.max(0, damage), target.maxHealth || GAME_CONFIG.MAX_HEALTH);
      ...
      if (target.health <= 0) {
        ...
        attacker.kills++;
        target.deaths++;
        this.scores[attacker.team] += this.mode === 'tdm' ? 1 : GAME_CONFIG.KILL_SCORE;
    ...
    respawnPlayer(playerId) {
      const player = this.players.get(playerId);
      if (player) {
        player.health = player.maxHealth || GAME_CONFIG.MAX_HEALTH;
    ...
    tickWindmills() {
      ...
      if (this.phase === 'intermission') {
        if (Date.now() >= this.intermissionUntil) this.startRound(this.round + 1);
        return;
      }
      if (this.isLive() && this.isRoundOver()) { this.endRound(); return; }
      if (!this.isLive() || this.mode !== 'domination') return;
      ...
      for (const mill of this.windmills) {
        ...
        const lowPass = { red: false, blue: false };
        for (const [, player] of this.players) {
          if (player.health <= 0 || !player.position) continue;
          const dx = player.position.x - mill.x;
          const dz = player.position.z - mill.z;
          const dist = Math.sqrt(dx * dx + dz * dz);
          if (dist <= GAME_CONFIG.CAPTURE_RADIUS) {
            // Bombers capture faster; a daring low pass doubles the rate
            let w = planeOf(player).captureWeight;
            if ((player.position.y || 30) < GAME_CONFIG.LOW_PASS_ALTITUDE) { w *= 2; lowPass[player.team] = true; }
            nearbyTeams[player.team] += w;
          }
        }
        ...
        const weight = Math.min(3, nearbyTeams[capturingTeam]);
        mill.lowPass = lowPass[capturingTeam];
        mill.progress = Math.min(1, mill.progress + GAME_CONFIG.CAPTURE_RATE * tickSeconds * weight);
        changed = true;

        if (mill.progress >= 1) {
          mill.team = capturingTeam;
          mill.progress = 1;
          mill.capturedAt = Date.now();
          mill.lowPass = false;
    ...
    tickWindmillScores() {
      ...
      for (const mill of this.windmills) {
        if (mill.team) {
          this.scores[mill.team] += this.suddenDeath ? 2 : 1;
          scored = true;
        }
      }
  ...
  function findOrCreateGame(mode) {
    for (const [id, game] of games) {
      ...
      if (game.mode === mode && game.realPlayerCount() < GAME_CONFIG.PLAYERS_PER_GAME && game.status === 'waiting') {
        return game;
      }
    }

    const gameId = `game_${Date.now()}`;
    const newGame = new Game(gameId, mode);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('joinGame', (payload) => {
      try {
        // Accept a bare username (older clients) or { username, mode }
        const username = typeof payload === 'string' ? payload : payload?.username;
        const mode = payload && payload.mode === 'tdm' ? 'tdm' : 'domination';
        const plane = payload && PLANES[payload.plane] ? payload.plane : DEFAULT_PLANE;
        const cosmetics = cleanCosmetics(payload && payload.cosmetics);
        ...
        const game = findOrCreateGame(mode);
        if (!game) {
        ...
        socket.emit('gameJoined', {
          ...
          gameState: {
            ...
            mode: game.mode,
            rules: {
              killScore: game.mode === 'tdm' ? 1 : GAME_CONFIG.KILL_SCORE,
              millTickSeconds: GAME_CONFIG.WINDMILL_SCORE_INTERVAL / 1000,
              matchDuration: GAME_CONFIG.MATCH_DURATION,
              tdmTarget: GAME_CONFIG.TDM_TARGET,
              streaks: [GAME_CONFIG.STREAK_RADAR, GAME_CONFIG.STREAK_WINGMAN, GAME_CONFIG.STREAK_AIRSTRIKE],
              roundsToWin: GAME_CONFIG.ROUNDS_TO_WIN,
              maxRounds: GAME_CONFIG.MAX_ROUNDS,
              intermission: GAME_CONFIG.INTERMISSION,
              lowPassAltitude: GAME_CONFIG.LOW_PASS_ALTITUDE,
              stealthAltitude: GAME_CONFIG.STEALTH_ALTITUDE,
              stealthReveal: GAME_CONFIG.STEALTH_REVEAL,
              planes: PLANES,
            },
            round: game.round,
     ...
     */
    /**
     * A human client saw its bullet pass through an enemy. Accept it when the
     * server's own picture roughly agrees (lag tolerance), then apply damage.
     */
    socket.on('hitClaim', (data) => {
      try {
        const playerInfo = players.get(socket.id);
        if (!playerInfo) return;
        const game = games.get(playerInfo.gameId);
        if (!game || game.status === 'ended' || !game.isLive()) return;
        const attacker = game.players.get(socket.id);
        if (!attacker || attacker.health <= 0) return;
        if (!data || !data.projectileId || !data.targetId || !data.position) return;

        const projectile = game.projectiles.get(data.projectileId);
        if (!projectile || projectile.ownerId !== socket.id) return; // unknown or already spent
        const target = game.players.get(data.targetId);
        if (!target || target.team === attacker.team || target.health <= 0 || !target.position) return;

        const p = data.position;
        if (!isValidPosition(p)) return;
        const dTarget = Math.hypot(p.x - target.position.x, (p.y || 0) - (target.position.y || 0), p.z - target.position.z);
        const dBullet = Math.hypot(p.x - projectile.position.x, (p.y || 0) - projectile.position.y, p.z - projectile.position.z);
        if (dTarget > GAME_CONFIG.CLAIM_TOLERANCE || dBullet > GAME_CONFIG.CLAIM_TOLERANCE * 1.5) return;

        game.projectiles.delete(data.projectileId);
        const claimDamage = planeDamage(attacker);
        const killed = game.handlePlayerHit(socket.id, data.targetId, claimDamage);
        ...
        console.error('Error in hitClaim:', error);
      }
    });
